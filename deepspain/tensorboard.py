...
class LearnerTensorboardWriter(LearnerCallback):
    ...
    def __init__(
        ...
        self.base_dir, self.name, self.loss_iters, self.hist_iters, self.stats_iters = (
            base_dir,
            name,
            loss_iters,
            hist_iters,
    ...
    def _update_batches_if_needed(self) -> None:
        ...
        if self.learn.data.valid_dl is None:
            return  # Running learning rate finder, so return
    ...
    def _write_model_stats(self, iteration: int) -> None:
        ...
        self.stats_writer.write(
            model=self._get_model(), iteration=iteration, tbwriter=self.tbwriter
        )
    ...
    def _write_metrics(
        self, iteration: int, last_metrics: MetricsList, start_idx: int = 2
    ) -> None:
        ...
        for i, name in enumerate(recorder.names[start_idx:]):
            ...
            self._write_scalar(
                name=name, scalar_value=scalar_value, iteration=iteration
            )
    ...
    def on_batch_end(
        self, last_loss: Tensor, iteration: int, train: bool, **kwargs
    ) -> None:
...
class ModelImageSet:
    ...
    def __init__(self, orig: Image, real: Image, gen: Image):
        self.orig, self.real, self.gen = orig, real, gen
...
class HistogramTBRequest(TBWriteRequest):
    ...
    def write(self) -> None:
        ...
        for param_name, values in self.params:
            self._write_histogram(param_name=param_name, values=values)
...
class HistogramTBWriter:
    ...
    def write(
        ...
        "Writes model histograms to Tensorboard."
        request = HistogramTBRequest(
            model=model, iteration=iteration, tbwriter=tbwriter, name=name
        )
...
class ModelStatsTBRequest(TBWriteRequest):
    ...
    def _add_gradient_scalar(self, name: str, scalar_value) -> None:
        ...
        self.tbwriter.add_scalar(
            tag=tag, scalar_value=scalar_value, global_step=self.iteration
        )
...
class ImageTBRequest(TBWriteRequest):
    ...
    def __init__(
        ...
        super().__init__(tbwriter=tbwriter, iteration=iteration)
        self.image_sets = ModelImageSet.get_list_from_model(
