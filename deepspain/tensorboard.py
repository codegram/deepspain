...
from fastai.callbacks.tensorboard import LearnerCallback
from typing import Any


class TensorBoard(LearnerCallback):
    def __init__(
        self,
        comment: str,
        learn,
        track_weight=False,
        track_grad=False,
        metric_names=["Validation loss", "Accuracy"],
    ):
        """
    ...
    def on_epoch_end(self, **kwargs: Any):
        ...
        if kwargs["last_metrics"][0] != None:
            ...
            self.writer.add_scalar(
                "Loss", kwargs["last_loss"], kwargs["epoch"] + self.delta_epochs
            )
            for i, met in enumerate(kwargs["last_metrics"]):
                self.writer.add_scalar(
                    self.metric_names[i], met, kwargs["epoch"] + self.delta_epochs
                )
    ...
    def on_backward_end(self, **kwargs: Any):
        if (
            self.track_grad
            and "last_metrics" in kwargs
            and kwargs["last_metrics"][0] != None
            and kwargs["epoch"] > self.last_epoch_backward
        ):
            ...
            for i, param in enumerate(self.learn.model.parameters()):
                ...
                if grad is not None:
                    self.writer.add_histogram(
                        "back" + "." + keys[i],
                        grad,
                        kwargs["epoch"] + self.delta_epochs,
                    )
                    i += 1
